      ...
      <div className="mt-8 space-y-6">
        {customerOrders.length === 0 ? (
          <div className="rounded-xl border border-border bg-white p-12 text-center">
            ...
            <Link href="/products" className="mt-4 inline-block rounded-lg bg-primary px-6 py-2.5 text-sm font-semibold text-white hover:bg-primary-dark transition-colors">
              Start Shopping
            </Link>
          </div>
        ) : (
          customerOrders.map((order) => (
            <div key={order.id} className="rounded-xl border border-border bg-white overflow-hidden">
